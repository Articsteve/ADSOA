...
public class MotherCell {
	...
	String cellInstanceFldr = null;
	String cellInstanceAdd = null;
	String cellInstanceSub = null;
	String cellInstanceMul = null;
	String cellInstanceDiv = null;
	...
	public MotherCell() {
		...
		this.cellInstanceFldr = "C:/temp/Cells/Cell-" + this.fingerPrint;
		new File(this.cellInstanceFldr).mkdirs();
		...
		switch( msg.getContentCode() ) {
			...
			case 15:{ 
				...
				if(msgArray[0] == '1') {
					...
					//if(msg.services[0] != null) {
						String tempStr = this.cellInstanceFldr + "/Add.jar";
						File from = new File(fileAdd);
						File to= new File(tempStr);
						try {
							Clonation.copy(from, to);
							this.cellInstanceAdd = tempStr;
						} catch (IOException e) {
							// TODO Auto-generated catch block
							e.printStackTrace();
						}
					...
					try {
			            File file = new File(this.cellInstanceAdd);
			            if (file.delete()) {
				...
				if(msgArray[1] == '1') {
					...
					System.out.println("Subtraction has been enabled for the cell " + this.fingerPrint);
					//if(msg.services[0] != null) {
						String tempStr = this.cellInstanceFldr + "/Sub.jar";
						File from = new File(fileSub);
						File to= new File(tempStr);
						try {
							Clonation.copy(from, to);
							this.cellInstanceSub = tempStr;
						} catch (IOException e) {
							// TODO Auto-generated catch block
							e.printStackTrace();
						}
					...
					try {
			            File file = new File(this.cellInstanceSub);
			            if (file.delete()) {
			                System.out.println("Successfuly deleted file: " + file.getName());
			            } else {
			                System.out.println("Failed: Delete operation is failed.");
			            }
			        } catch (Exception e) {}
				...
				if(msgArray[2] == '1') {
					...
					String tempStr = this.cellInstanceFldr + "/Mul.jar";
					File from = new File(fileSub);
					File to= new File(tempStr);
					try {
						Clonation.copy(from, to);
						this.cellInstanceMul = tempStr;
					} catch (IOException e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
					}
					...
					try {
			            File file = new File(this.cellInstanceMul);
			            if (file.delete()) {
			                System.out.println("Successfuly deleted file: " + file.getName());
			            } else {
			                System.out.println("Failed: Delete operation is failed.");
				...
				if(msgArray[3] == '1') {
					...
					System.out.println("Division has been enabled for the cell " + this.fingerPrint);
					String tempStr = this.cellInstanceFldr + "/Div.jar";
					File from = new File(fileSub);
					File to= new File(tempStr);
					try {
						Clonation.copy(from, to);
						this.cellInstanceDiv = tempStr;
					} catch (IOException e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
					}
					...
					try {
			            File file = new File(this.cellInstanceDiv);
			            if (file.delete()) {
			                System.out.println("Successfuly deleted file: " + file.getName());
			            } else {
			                System.out.println("Failed: Delete operation is failed.");
